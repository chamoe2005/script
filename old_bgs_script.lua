...
local newEvent = {["Name"] = "Carnival Event", ["Currency"] = "Tokens", ["Image"] = "rbxassetid://5410765919", ["Color"] = Color3.fromRGB(255, 100, 255)}
--local newEvent = {}
...
if _G.InfoPanelOn then	
   ...
   -- misc:Button('Swap World', function()
	...
	misc:Section("Misc")
    misc:Toggle("Auto Equip Enabled", {flag = 'AutoEquip'})
    --misc:Box('Num. Pets', {
        --flag = "numPets",
        --type = 'number'
    --})
	misc:Dropdown("Equip Currency", {
		location = _G,
        flag = "buffCurrency",
        list = currencylist},
	...
	misc:Toggle("Collect Pet Rewards", {flag = 'AutoWorldRewards'})
	...
	spawn(function()
		while wait(1) do
			if misc.flags.AutoWorldRewards then
				local claimedRewardsIndex = require(game:GetService("ReplicatedStorage").Assets.Modules.Library.index)["REWARDS"]
				local playerData = f:InvokeServer("GetPlayerData")
				local rewards = require(game:GetService("ReplicatedStorage").Assets.Modules.ItemDataService.RewardsModule)

				for i,d in pairs(rewards.Worlds) do
					local currentReward = playerData[claimedRewardsIndex][i]
					if currentReward < d.MaxLevel then
						if rewards.GetCost(rewards, i, currentReward + 1) < currencys[d.Currency].Value then
							f:FireServer("CollectReward",i,currentReward + 1)
							wait(.5)
						end
					end
				end
	...
	--eggs:Box('Egg Delay (w/Farm)', {flag = 'eggdelay', type = 'text'})
...
--	_G.eggCounter = 0
    ...
    spawn(function()
        while wait(.1) do
			...
			--local eggTimer = eggs.flags.eggdelay
			--if tonumber(eggTimer) ~= nil and farm.flags.Drops == true then
				--if os.time() >= (tonumber(eggTimer) + _G.lastEgg) then
					--_G.eggSkip = false
				--else
					--_G.eggSkip = true
				--end
		...
		--local numEggs = playerData[eggsOpenedindex]
		
		local canaffordold = _G.canafford
			if not _G.sell and not _G.collectingchests and not _G.autoguessing and (eggs.flags.eggs == true or eggs.flags.three == true) then --and not _G.eggSkip then
				--local currentWorld = _G.world
								...
								if eggtps[eggs.flags.egg] then
									if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
										...
										--end
												...
												--if playerData[eggsOpenedindex] > _G.eggCounter or _G.eggCounter == 0 then
													--_G.lastEgg = os.time()
													--_G.eggCounter = playerData[eggsOpenedindex]
												--end
									end
								end
							else
	...
	farm:Toggle('Redeem Carnival Chal', {flag = 'carnivalChal'})
   ...
   -- farm:Toggle('Friendly AutoGuess', {flag = 'Friendly'})
    ...
    spawn(function()
		 while wait(.1) do
			...
			if farm.flags.carnivalChal then
				f['FireServer'](f, 'CompleteCarnivalChallenge', 'Easy')
				wait(1)
				f['FireServer'](f, 'CompleteCarnivalChallenge', 'Medium')
				wait(1)
				f['FireServer'](f, 'CompleteCarnivalChallenge', 'Hard')
				wait(1)
			end	
    ...
    spawn(function ()
    while wait() do 
    	if not _G.sell and (farm.flags.Drops or target) then
        ...
        while wait(1) do
			...
			for i,d in pairs(e[pn]) do
				...
				if _G.AutoInv == "Delete Pets" and not d[7] then				
					...
					if deleted == false and _G.InfoPanelOn then
			...
			if _G.InfoPanelOn and _G.newInv[1] ~= nil then
	...
	local wallyGUI = game:GetService("CoreGui").ScreenGui:GetDescendants()

	if _G.bubble then
		farm.flags.Bubble = true
		local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Auto-Bubble Blow", true)
		textbox.Checkmark.Text = utf8.char(10003)
	end
	
	if _G.autoEquip ~= "" then
		local buffTextBox
	
		for i,d in pairs(wallyGUI) do
			if d.Parent.Name == "dropdown_lbl" and d.Text == "Bubbles" then
				buffTextBox = d
			end
		end
		
		misc.flags.AutoEquip = true
		changeBuff("Bubbles", _G.autoEquip)
		local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Auto Equip Enabled", true)
		textbox.Checkmark.Text = utf8.char(10003)
		buffTextBox.Text = _G.autoEquip
	end
		
	
	if _G.startWorld ~= "" then
		tptoDrops("Overworld", _G.startWorld)
		
		local worldTextBox
	
		for i,d in pairs(wallyGUI) do
			if d.Parent.Name == "dropdown_lbl" and d.Text == "Overworld" then
				worldTextBox = d
			end
		end

		worldTextBox.Text = _G.startWorld
		wait(1)
	end
	
		
	if _G.open3 ~= "" or _G.open1 ~= "" then
		cccc.HatchEgg = function()end
		cccc.MultiHatchEgg = function()end
		local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Skip Animation", true)
		textbox.Checkmark.Text = utf8.char(10003)

		if _G.open3 ~= "" then
			eggs.flags.three = true
			eggs.flags.egg = _G.open3
			local eggTextBox
			for i,d in pairs(wallyGUI) do
				if d.Name == "Box" and d.PlaceholderText == "Egg" then
					eggTextBox = d
				end
			end
			eggTextBox.PlaceholderText = _G.open3
			local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Buy Eggs (3x)", true)
			textbox.Checkmark.Text = utf8.char(10003)
		else
			eggs.flags.eggs = true
			eggs.flags.egg = _G.open1
			local eggTextBox
			for i,d in pairs(wallyGUI) do
				if d.Name == "Box" and d.PlaceholderText == "Egg" then
					eggTextBox = d
			...
			eggTextBox.PlaceholderText = _G.open1
			local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Buy Eggs", true)
			textbox.Checkmark.Text = utf8.char(10003)
		end
	end
	
	if _G.autoChestSetting ~= "" then
		if _G.currencyOverride then
			misc.flags.ChestOver = true
			local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("AutoChest Override", true)
			textbox.Checkmark.Text = utf8.char(10003)
		end
		_G.giantchestcollected = 0 
		_G.mediumchestcollected = 0
		_G.smallchestcollected = 0
		_G.coinchestcollected = 0
		_G.autochest = _G.autoChestSetting
		
		local autoChestTextBox
	
		for i,d in pairs(wallyGUI) do
			if d.Parent.Name == "dropdown_lbl" and d.Text == "Auto Chest Off" then
				autoChestTextBox = d
			end
		end
	
		autoChestTextBox.Text = _G.autoChestSetting
	end	

	
	if _G.match then
		farm.flags.matchpets = true
		local textbox = game:GetService("CoreGui").ScreenGui.Container:FindFirstChild("Match Pets", true)
		textbox.Checkmark.Text = utf8.char(10003)
	end
end
